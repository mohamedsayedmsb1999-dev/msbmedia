...
export default function ConsultantAssistant() {
  ...
  return <div className="assistant-root">
    {open && <section className="assistant-window" aria-label="خبير مبيعات MSB Media الذكي">
      ...
      {handoffReady ? <button onClick={() => openWhatsApp(`ملخص احتياجي من مساعد MSB Media: ${leadSummary}`)} className="assistant-handoff"><MessageCircle className="h-4 w-4 text-[#25D366]" />ابدأ محادثة واتساب مع الفريق</button> : <p className="assistant-stage-note">اسألني عن هدفك أو نوع الخدمة، وبعدها نرتب لك الخطوة المناسبة.</p>}
